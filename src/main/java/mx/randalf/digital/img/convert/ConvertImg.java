...
public class ConvertImg {
	...
	public void resize(File input, File output, int ppi) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, 0, 0);
	}

	public void resize(File input, File output, int ppi, Integer resize) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, 0, 0, resize);
	}

	public void resize(File input, File output, int ppi, Double quality) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, 0, 0, quality);
	}

	public void resize(File input, File output, int ppi, Double quality, Integer resize) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, 0, 0, quality, resize);
	}

	public void resize(File input, File output, int ppi, int width, int height) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, width, height, new Double("60"));
	}

	public void resize(File input, File output, int ppi, int width, int height, Integer resize) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, width, height, new Double("60"), resize);
	}

	public void resize(File input, File output, int ppi, int width, int height, Double quality) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		resize(input, output, ppi, width, height, quality, null);
	}

	public void resize(File input, File output, int ppi, int width, int height, Double quality, Integer resize) throws NumberFormatException, IOException, InterruptedException, IM4JavaException{
		ConvertCmd cc = null;
		...
		try {
			if (input.exists()){
				if (!output.getParentFile().exists()){
					if (!output.getParentFile().mkdirs()){
						throw new IOException("Riscontrato un problema nella creazione della cartella ["+output.getParentFile().getAbsolutePath()+"]");
					}
				}
				...
				if (ppi!= resolution){
					...
					if (resolution>ppi){
						imo.resize(info.getImageWidth()/(resolution/ppi), info.getImageHeight()/(resolution/ppi));
					}else {
						imo.resize(info.getImageWidth()*(ppi/resolution), info.getImageHeight()*(ppi/resolution));
			...
			if (fTmp != null &&
					fTmp.exists()){
				if (!fTmp.delete()){
					throw new IOException("Riscontrato un problema nella cancellazione del file ["+fTmp.getAbsolutePath()+"]");
				}
			}
		}
	}
	...
	public String setStpiecePer(int anno, int mese, int giorno, String volume, String numero){
		String stpiecePer = "";
	...
	public String setStpiecePer(int anno, int mese, String giorno, String volume, String numero){
		String stpiecePer = "";
		DecimalFormat df4 = new DecimalFormat("0000");
		DecimalFormat df2 = new DecimalFormat("00");
		boolean sup = false;
