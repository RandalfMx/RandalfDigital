...
public class ConvertImg {
    ...
    try {
      ...
      if (!fConvert.exists()) {
        throw new FileNotFoundException(
            "Impossibile trovare il file " + fConvert.getAbsolutePath());
      }
  ...
  public void resize(File input, File output, int ppi, Double quality,
      Integer resize) throws NumberFormatException, IOException,
  ...
  public void resize(File input, File output, int ppi, int width, int height,
      Double quality) throws NumberFormatException, IOException,
      InterruptedException, IM4JavaException {
    resize(input, output, ppi, width, height, quality, null);
  }

  public void resize(File input, File output, int ppi, int width, int height,
      Double quality, Integer resize) throws NumberFormatException, IOException,
      InterruptedException, IM4JavaException {
    ConvertCmd cc = null;
    ...
    try {
      if (input.exists()) {
        ...
        throw new FileNotFoundException(
            "Il file [" + input.getAbsolutePath() + "] non esiste");
      }
  ...
  private void convertSrgb(File input, File output)
  ...
  public static String setStpiecePer(int anno, int mese, int giorno,
      String annata, String fascicolo, String edizione) {
    String stpiecePer = "";
    String fasc = "";
    int iAnnata = 0;
      ...
      for (int x = 0; x < st.length; x++) {
        try {
          ...
          if (st[x].equals("[ed.") || st[x].equals("[I")) {
            ...
            stpiecePer += ":03*";
          } else if (st[x].equals("str.]") || st[x].equals("ed.straordinaria]")
              || st[x].equals("stra.]")) {
          } else {
            stpiecePer += st[x];
          }
  ...
  public static String setStpiecePer(int anno, int mese, String giorno,
      String volume, String numero) {
    String stpiecePer = "";
    DecimalFormat df4 = new DecimalFormat("0000");
    DecimalFormat df2 = new DecimalFormat("00");
  ...
  public void crop(File input, File output, int larghezza, int altezza, int x,
      int y) throws IOException, InterruptedException, IM4JavaException {
    ConvertCmd cc = null;
    IMOperation imo = null;
